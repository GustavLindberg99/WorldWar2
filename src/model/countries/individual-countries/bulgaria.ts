...
export default class Bulgaria extends CountryWithUnits {
    constructor(){
        super(Partnership.Axis);
        this.availableUnits = new Set(new Array(15).fill(null).map(() => new Infantry(1, 3, this)));
    }
    ...
    override canBeActivated(partnership: Partnership): boolean {
        return this.partnership() === Partnership.Neutral
            ...
            && Countries.germany.landUnits().some(it => it.hex()?.country === Countries.greece || it.hex()?.country === Countries.yugoslavia);
    }
